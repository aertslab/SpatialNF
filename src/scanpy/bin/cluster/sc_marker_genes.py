...
num_cell_by_clusters = adata.obs.loc[:, args.groupby].value_counts()
if np.sum(num_cell_by_clusters == 1) > 0:
    clusters_to_keep = num_cell_by_clusters[num_cell_by_clusters > 1].index
    adata = adata[adata.obs[args.groupby].isin(clusters_to_keep)]

print(f"Ranking genes for characterizing groups (aka sc.tl.rank_genes_groups) using {args.method} method...")
adata.obs.loc[:,args.groupby] = adata.obs.loc[:,args.groupby].astype('category')
KEY_ADDED_STRING = 'rank_genes_groups' if (args.groupby=='louvain') | (args.groupby=='leiden') else 'rank_genes_groups_' + args.groupby
