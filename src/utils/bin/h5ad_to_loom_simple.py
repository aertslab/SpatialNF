...
parser.add_argument(
    ...
    choices=['X', 'raw_X'],
    default='X',
    help='The slot name where to find the raw matrix in adata. X refers to adata.X while X_raw refers to adata.raw.X.'
)
...
try:
    adata = read_h5ad(file_path=FILE_PATHS_IN[0].name, backed=None)
...
for adata_idx in range(0, len(FILE_PATHS_IN)):
    ...
    for i in range(0, num_clusters):
        ...
        marker_genes_along_adata_mask = np.in1d(
            adata.var.index,
            marker_names
        )
        marker_genes_along_adata = cluster_markers.index[marker_genes_along_adata_mask]
